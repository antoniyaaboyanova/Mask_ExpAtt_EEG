...
def check_eye_in_location(el_tracker, x_location, y_location, eye_used,
                           diam_fixation_control, ppd, beta=None, sample=None):
    ...
    try:
        ...
        if eye_used == 1 and sample.isRightSample():
            eye_x, eye_y = sample.getRightEye().getGaze()
        elif eye_used == 0 and sample.isLeftSample():
            eye_x, eye_y = sample.getLeftEye().getGaze()
        ...
        if eye_x == pylink.MISSING_DATA or eye_y == pylink.MISSING_DATA:
            return False, None, None
        ...
        radius = (diam_fixation_control * ppd) / 2
        distance = math.sqrt((eye_x - x_location) ** 2 + (eye_y - y_location) ** 2)
        in_location = distance <= radius

        return in_location, eye_x, eye_y

    except Exception as e:
        print(f"check_eye_in_location error: {e}")
        return False, None, None
