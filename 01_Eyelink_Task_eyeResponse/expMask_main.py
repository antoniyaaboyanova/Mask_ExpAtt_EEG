...
while True:
    ...
    myDlg.addField("Task Type:", choices=["mix", "loc", "ide"])
    ok_data = myDlg.show()
...
# ── PPD calculation from monitor parameters ───────────────────────────────────
# These should match your mon = monitors.Monitor(...) setup
screen_width_cm  = mon.getWidth()       # physical screen width in cm
screen_dist_cm   = mon.getDistance()    # eye-to-screen distance in cm
screen_width_pix = win.size[0]          # horizontal resolution in pixels

# Degrees subtended by the full screen width
screen_width_deg = 2 * math.degrees(math.atan(screen_width_cm / 2 / screen_dist_cm))

# Pixels per degree
ppd = screen_width_pix / screen_width_deg

print(f"Monitor: {screen_width_pix}px wide, {screen_width_cm}cm wide, "
      f"{screen_dist_cm}cm distance → {ppd:.2f} ppd")
